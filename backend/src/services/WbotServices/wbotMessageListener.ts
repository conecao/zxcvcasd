...
export const getBodyMessage = (msg: proto.IWebMessageInfo): string | null => {
  try {
    ...
    const types = {
      ...
      templateMessage:  msg.message?.templateMessage?.fourRowTemplate?.imageMessage || msg.message?.templateMessage?.hydratedTemplate?.imageMessage || msg.message?.templateMessage?.hydratedFourRowTemplate?.imageMessage 
    };
...
const handleMessage = async (
  ...
  try {
    ...
    if (msg.key.fromMe && count?.unreadCount > 0) {
      ...
      SendAckBYRemoteJid({ remoteJid });
    }

    let unreadMessages = msg.key.fromMe ? 1 : count?.unreadCount || 1;
